...
from get_datasets import get_args_USA
...
def instance_selection(cls_prob, num, instance_weights1, instance_weights2):
    ...
    cls_label = cls_prob.argmax(dim=1)
    pos_indices = torch.nonzero(cls_label==0)
    neg_indices = torch.nonzero(cls_label==1)
    pos_values, pos_selected_indices = instance_weights[pos_indices.reshape(-1)].topk(np.min([int(num*0.25), instance_weights[pos_indices.reshape(-1)].shape[0]]), dim=0)
    neg_values, neg_selected_indices = instance_weights[neg_indices.reshape(-1)].topk(np.min([int(num*0.75), instance_weights[neg_indices.reshape(-1)].shape[0]]), dim=0)
    instance_weights = torch.zeros_like(instance_weights)
    instance_weights[pos_indices[pos_selected_indices.reshape(-1)].reshape(-1)] = pos_values
    instance_weights[neg_indices[neg_selected_indices.reshape(-1)].reshape(-1)] = neg_values
